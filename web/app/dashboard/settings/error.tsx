...
export default function SettingsError({ error, reset }: SettingsErrorProps) {
  return (
    <section className="rounded-[8px] border border-brand-border bg-brand-card px-6 py-10">
      <h1 className="text-[20px] font-semibold text-white">Unable to load settings</h1>
      <p className="mt-2 text-[13px] text-[#a1a1aa]">{error.message || "The settings page failed to load."}</p>
      <button
        type="button"
        onClick={reset}
